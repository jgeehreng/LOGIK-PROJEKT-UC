...
check_if_running() {
    if pgrep -f "$archive_script_path" > /dev/null; then
        echo "Script $archive_script_path is already running. Exiting."
        exit 1
    fi
}
...
create_crontab_entry() {
    local cron_time=$1
    local script_path=$2
    local log_dir=$archive_script_cron_log_dir
...
if [[ -n "$cron_time" ]]; then
    mkdir -p "$archive_script_cron_log_dir"
    create_crontab_entry "$cron_time" "$archive_script_path" "$archive_script_cron_log_dir"
else
